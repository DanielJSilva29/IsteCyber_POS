...
class ManagementPage(QWidget):
    ...
    def _build(self):
        ...
        users_card.setStyleSheet("#usersCard { background-color: #2a2a2a; border-radius: 10px; border: 1px solid #444; padding: 10px; }")
        ...
        prod_card.setStyleSheet("#productsCard { background-color: #2a2a2a; border-radius: 10px; border: 1px solid #444; padding: 10px; }")
    ...
    def _refresh_lists(self):
        ...
        for p in self.sm.list_products():
            ...
            desc = f'{p["name"]} ({p["ptype"]}) {p.get("price_no_vat", 0):.2f}€'
            item = QListWidgetItem(desc)
            # USAR O HELPER AQUI
            item.setIcon(self._load_icon(p.get("image_path")))
    ...
    def _add_product_dialog(self):
        ...
        ptype, ok = QInputDialog.getItem(self, "Produto", "Tipo:", options, 0, False)
        if not ok: return

        image_path, _ = QFileDialog.getOpenFileName(self, "Imagem do Produto (opcional)", "images", "Imagens (*.png *.jpg *.jpeg)")

        prod = Product(
            code=code, name=name, price_no_vat=float(price), ptype=ptype,
    ...
    def _edit_product_dialog(self, item):
        ...
        prod = None
        for p in products:
            p_desc = f'{p["name"]} ({p["ptype"]}) {p.get("price_no_vat", 0):.2f}€'
            if p_desc == desc:
                prod = p
                break
        ...
        if not ok or not name: return
        price, ok = QInputDialog.getDouble(self, "Editar produto", "Preço s/IVA:", float(prod.get("price_no_vat", 0.0)), 0)
        if not ok: return
        ...
        ptype, ok = QInputDialog.getItem(self, "Editar produto", "Tipo:", options, current_index, False)
        if not ok: return

        image_path, _ = QFileDialog.getOpenFileName(self, "Nova imagem (opcional)", "images", "Imagens (*.png *.jpg *.jpeg)")
        updates = {"name": name, "price_no_vat": float(price), "ptype": ptype}
        if image_path:
            updates["image_path"] = image_path
        
        try:
            self.sm.update_product(prod["code"], **updates)
