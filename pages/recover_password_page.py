...
class RecoverPasswordPage(QWidget):
    ...
    def _reset(self):
        """Valida o código e redefine a password do utilizador alvo."""
        ...
        # Usar a API pública do SystemManager
        users = self.sm.list_users()
        username_target = self.target_user.get("username")

        for u in users:
            if not isinstance(u, dict):
                continue
            if u.get("username") == username_target:
        ...
        # Reset do estado interno e campos
        self.code = None
        self.target_user = None
        self.email.clear()
